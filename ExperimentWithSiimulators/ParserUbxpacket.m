    ...
    while(portPos < portCnt)
        ...
        while(pos < (size - packet_len) )
            if((data(pos) == MAGIC0) & (data(pos + 1) == MAGIC1))
                ...
                class_id_len = 2; length_len = 2; checksum_len = 2;
%                         dec2hex(data(pos + 2 + (0 : 4)))
                crc_is_right = CheckCRC(data(pos + (2 : (2 + class_id_len + ...
                                                  length_len + length + ...
                                                  checksum_len -1))), length);
                if (crc_is_right)
                    switch (command)
...
function crc_is_good = CheckCRC(data, len)
    ck_a = 0; ck_b = 0;
    ck_a = ck_a + data(1);
    ck_b = ck_b + ck_a;
